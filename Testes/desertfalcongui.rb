...
class DesertFalconGUI < Gosu::Window # Janela do jogo, com script de seleção de personagem e script de operação do jogo, tanto visual quando lógico
  ...
  def initialize
    ...
    @playerOptionList = ["falcon", "milenium", "bat", "rocket", "arrow", "plane", "chopper", "desertwasp", "squirrel", "omniscientredorb", "jetpackturtle", "paperplane"] # Opçoes de personagem pro jogador
  ...
  def update # Enquanto não for selecionado um personagem, rodar a tela de seleção de personagems. Após escolher um personagem, colocar esse personagem no jogo e rodar os frames do jogo
    if (@playerSelect == true and @paused == false and @slowmo == false) # Caso não tenha selecionado personagem, rodar script de seleção, senão, rode script do jogo
  ...
  def draw # Caso não tenha sido selecionado um personagem, mostrar tela de seleção com botões pra cada opção. Caso tenha sido selecionado um personagem, printar objetos do jogo nas suas respectivas posições
    ...
    if (@boopIterator  ==  @@FPS_COUNTER_PERIOD)
      @boopIterator = 0
      @FPScount = @@FPS_COUNTER_PERIOD / (((Time.now.to_f * 1000) - @sysTime) / 1000) # Quando boop terminar seu intervalo, é calculado uma nova média de fps durante esse tempo
    ...
    if (@playerSelect == true) # Se já foi escolhido um personagem, renderizar objetos do jogo
      ...
      @playerOptionList.each do # Para cada opção
        ...
        if (indexaux == @selectopt)
          Gosu::Image.new("./programicons/" + "selectorarrow.png").draw((((indexaux - 1) * @@BUTTON_SIZE) + @@MARGIN_OFFSET) - (((indexaux - 1) / 12) * (@@BUTTON_SIZE * 12)), @@MARGIN_OFFSET + ((@@BUTTON_SIZE) * ((indexaux - 1) / 12)), 2, 1, 1) # Caso seja a opção selecionada atualmente, mostre uma seta para deixar explicito para o usuário
        end
